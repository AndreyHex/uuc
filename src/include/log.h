...
#define LOG_ERROR(str, ...) do { printf("[ERROR] [%s:%d] ", __FILE__, __LINE__);printf(str, ##__VA_ARGS__); } while(0);
#else
#define LOG_INFO(str, ...) do { printf("[INFO] ");printf(str, ##__VA_ARGS__); } while(0);
#define LOG_ERROR(str, ...) do { printf("[ERROR] ");printf(str, ##__VA_ARGS__); } while(0);
#endif
